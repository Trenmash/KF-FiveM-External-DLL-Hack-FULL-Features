...
namespace ImGui
{
    ...
    IMGUI_API bool          artab(const char* label, bool selected);
    IMGUI_API bool          arsubtab(const char* label, bool selected);
    // Content region
...
bool icon_box(ImTextureID id_image, ImVec2 size, ImVec2 icon_size, ImU32 color_icon);
// button
...
void Combo_Item(const char * Label, int * items, const char * preview[], int item);
bool Checkbox(const char * label, bool * v, bool offset);
// square button with an arrow shape
    ...
    IMGUI_API bool          Hotkey(const char* label, int* k, const ImVec2& size_ar);
    // Clipboard Utilities
