...
bool ImGui::Keybind(const char* label, int* key, int* mode, bool show_label)
{
    ...
    const float width = (GetContentRegionMax().x - 55);

    const ImRect rect(window->DC.CursorPos + ImVec2(25, 0), window->DC.CursorPos + ImVec2(width + 25, 20.f));
    ...
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0, 1), clickable.Max - ImVec2(0, 2), GetColorU32(c::keybind::background_inactive), c::all_rounding);
...
void ImGui::GradientText(const char* label, ImVec2 pos, ImU32 one_color, ImU32 two_color)
{

    const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
    ImGui::GetWindowDrawList()->AddText(pos, ImGui::GetColorU32(one_color), label);
    ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImGui::GetWindowDrawList(), vtx_idx_1, ImGui::GetWindowDrawList()->VtxBuffer.Size, pos, pos + ImVec2(0, ImGui::CalcTextSize(label).y), ImGui::GetColorU32(one_color), ImGui::GetColorU32(two_color));
...
bool ImGui::Tab(bool selected, ImTextureID image, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.image = ImLerp(it_anim->second.image, selected || hovered ? c::accent : c::tab::background_inactive, g.IO.DeltaTime * 12.f);
    it_anim->second.shadow = ImLerp(it_anim->second.shadow, selected ? c::shadow : c::shadow_invisible, g.IO.DeltaTime * 12.f);
    it_anim->second.shadow_offset = ImLerp(it_anim->second.shadow_offset, selected ? 60.f : 20.f, g.IO.DeltaTime * 8.f);

    ImGui::GetWindowDrawList()->AddImage(image, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.image));

    ImGui::GetWindowDrawList()->AddShadowCircle(ImVec2(bb.Min.x, bb.Min.y) + size_arg / 2, 3.f, GetColorU32(it_anim->second.shadow), it_anim->second.shadow_offset, ImVec2(0, 0), NULL, 100);
...
bool ImGui::SubTab(bool selected, const char* label, const ImVec2& size_arg)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? c::tab::background_gradient0 : c::tab::background_gradient1, g.IO.DeltaTime * 12.f);
    it_anim->second.size_x = ImLerp(it_anim->second.size_x, selected ? size_arg.x : 0.f, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::accent : c::text::text, g.IO.DeltaTime * 12.f);

    ImGui::GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max - ImVec2(size_arg.x - it_anim->second.size_x, 0), GetColorU32(it_anim->second.background), GetColorU32(c::tab::background_gradient1), GetColorU32(c::tab::background_gradient1), GetColorU32(it_anim->second.background), 0.f);

    PushFont(font::open_bold);
    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 20, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);
    PopFont();
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    RenderArrow(GetWindowDrawList(), ImVec2(bb.Min.x - 17, bb.Max.y - 10), GetColorU32(c::scroll::background), ImGuiDir_Down, 0.6f);

    RenderArrow(GetWindowDrawList(), ImVec2(bb.Min.x - 17, bb.Min.y), GetColorU32(c::scroll::background), ImGuiDir_Up, 0.6f);

    GetWindowDrawList()->AddRectFilled(grab_rect.Min - ImVec2(7.3f, -15), grab_rect.Max - ImVec2(14.5f, 15), GetColorU32(c::scroll::background), c::all_rounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::accent : c::checkbox::background_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.checkmark = ImLerp(it_anim->second.checkmark, *v ? c::checkbox::mark_active : c::checkbox::mark_inactive, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background), c::all_rounding);

    RenderCheckMark(GetWindowDrawList(), check_bb.Min + ImVec2(square_sz / 2, square_sz / 2) / 2, GetColorU32(it_anim->second.checkmark), square_sz / 2);

    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x + 10, check_bb.Min.y - 1), GetColorU32(it_anim->second.text), label);
...
struct combo_state
{
    ImVec4 background, combo_icon, text;
};

namespace image
{
    extern ID3D11ShaderResourceView* combo_ico;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = GetContentRegionMax().x - 47;

    const ImRect bb(window->DC.CursorPos + ImVec2(25, 20), window->DC.CursorPos + ImVec2(w + 17, 40));
    const ImRect total_bb(bb.Min, bb.Max + ImVec2(label_size.x, 0.0f));

    ItemSize(ImRect(total_bb.Min, total_bb.Max + ImVec2(0, 22)));
    ...
    it_anim->second.combo_icon = ImLerp(it_anim->second.combo_icon, popup_open ? c::accent : c::combo::icon_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, popup_open ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

    window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), GetColorU32(c::combo::background_inactive), c::all_rounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);

    window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, GetColorU32(c::combo::background_inactive), c::all_rounding, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
    ...
    GetWindowDrawList()->AddText(ImVec2(bb.Max.x - w + 8, bb.Min.y - 22), GetColorU32(it_anim->second.text), label);

    GetWindowDrawList()->AddImage(image::combo_ico, ImVec2(bb.Max.x - 17, bb.Min.y + 3), ImVec2(bb.Max.x - 3, bb.Min.y + 17), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.combo_icon));
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_PopupBg, GetColorU32(c::combo::list_inactive));
...
struct slider_state
{
    ImVec4 background = c::slider::background_inactive;
    ImVec4 text_num = c::text::text;
    ImVec4 text = c::text::text;
    ImVec4 navigator = c::slider::navigator_inactive;
    float slow, up_text;
};
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = GetContentRegionMax().x - 55;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(25, 22), window->DC.CursorPos + ImVec2(w + 25, 27));
    ...
    it_anim->second.navigator = ImLerp(it_anim->second.navigator, IsItemActive() ? c::accent : c::slider::navigator_inactive, g.IO.DeltaTime * 6.f);

    it_anim->second.text_num = ImLerp(it_anim->second.text_num, IsItemActive() ? c::slider::navigator_text : c::text::text_hide, g.IO.DeltaTime * 6.f);
    ...
    it_anim->second.up_text = ImLerp(it_anim->second.up_text, IsItemActive() ? 0.f : 15.f, g.IO.DeltaTime * 8.f);

    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::slider::background_inactive), c::all_rounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 0), ImVec2(it_anim->second.slow + frame_bb.Min.x + 14, grab_bb.Max.y + 1), GetColorU32(c::accent), c::all_rounding);
    }

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w - 1, frame_bb.Min.y - 23), GetColorU32(it_anim->second.text), label);

    RenderArrow(GetWindowDrawList(), ImVec2(it_anim->second.slow + frame_bb.Min.x + 5, grab_bb.Max.y - 20 + it_anim->second.up_text), GetColorU32(it_anim->second.navigator), ImGuiDir_Down, 0.8f);

    GetWindowDrawList()->AddRectFilled(ImVec2(it_anim->second.slow + frame_bb.Min.x + 8 - (CalcTextSize(value_buf).x / 2), grab_bb.Min.y - 27 + it_anim->second.up_text), ImVec2(it_anim->second.slow + frame_bb.Min.x + 16 + (CalcTextSize(value_buf).x / 2), grab_bb.Max.y - 15 + it_anim->second.up_text), GetColorU32(it_anim->second.navigator), 2.f);

    PushFont(font::open_bold);
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_num));
    RenderTextClipped(frame_bb.Min + ImVec2(it_anim->second.slow - 20, -42 + it_anim->second.up_text), ImVec2(it_anim->second.slow + frame_bb.Min.x + 44, grab_bb.Max.y + 1 + it_anim->second.up_text), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    PopStyleColor();
    PopFont();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(25, 20), window->DC.CursorPos + frame_size + ImVec2(17 , 20));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.icon = ImLerp(it_anim->second.icon, IsItemActive() ? c::accent : c::input::icon_inactive, g.IO.DeltaTime * 6.f);

    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::input::background_inactive), c::all_rounding);
        GetWindowDrawList()->AddImage(image::combo_ico, ImVec2(frame_bb.Max.x - 17, frame_bb.Min.y + 3), ImVec2(frame_bb.Max.x - 3, frame_bb.Min.y + 17), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x - 30, frame_bb.Min.y + inner_size.y);
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(5, (size_arg.y - CalcTextSize(hint).y) / 2);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(c::text::text_active));
    ...
    if (g.LogEnabled && (!is_password || is_displaying_hint))
    {
        ...
        GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - (size_arg.x - 8), frame_bb.Min.y - 21), GetColorU32(it_anim->second.text), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleVar(ImGuiStyleVar_PopupRounding, c::all_rounding);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(24, 1), GetColorU32(c::text::text), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 bar_pos = window->DC.CursorPos + ImVec2(0, 126);
    ...
    // Render SV Square
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, c::all_rounding, ImDrawFlags_RoundCornersTop);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, c::all_rounding, ImDrawFlags_RoundCornersTop);
    ...
    // Render Hue Bar
    for (int i = 0; i < 6; ++i)
        GetForegroundDrawList()->AddRectFilledMultiColor(ImVec2(bar0_pos_x + i * (bars_width / 6), bar_pos.y + 4), ImVec2(bar0_pos_x + (i + 1) * (bars_width / 6), bar_pos.y + sv_bar_size - 5), col_hues[i], col_hues[i + 1], col_hues[i + 1], col_hues[i]);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, bar_pos.y + 4), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, c::all_rounding, ImDrawCornerFlags_Bot);
    ...
    GetForegroundDrawList()->AddRectFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x - 3, bar_pos.y + 3), ImVec2(it_anim->second.hue_bar + bar_pos.x + 5, bar_pos.y + 19), ImColor(255, 255, 255, 255), 3.f);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, c::all_rounding, ImDrawCornerFlags_All);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const float width = GetContentRegionMax().x - 32;
    ...
    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max + ImVec2(2, 0), GetColorU32(col_rgb), 6.f, ImVec2(0.f, 0.f), c::all_rounding);
